...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  String getConvertedPrice(String priceStr) {
    ...
    final rateIDR = conversionRates['IDR']!;
    final rateTarget = conversionRates[widget.selectedCurrency]!;

    final converted = basePrice / rateIDR * rateTarget;
  ...
  double calculateTotalConverted(List<CartItem> items) {
    final rateIDR = conversionRates['IDR']!;
    final rateTarget = conversionRates[widget.selectedCurrency]!;

    double totalIDR = items.fold(0.0, (sum, item) {
      final price = double.tryParse(item.price) ?? 0.0;
      return sum + (price * item.quantity);
    });

    return totalIDR / rateIDR * rateTarget;
  }
  ...
  String _formatPrice(double price) {
    final symbol = widget.selectedCurrency == 'IDR'
        ...
        : widget.selectedCurrency == 'USD'
            ? '\$'
            : '€';
    return '$symbol${price.toStringAsFixed(widget.selectedCurrency == 'IDR' ? 0 : 2)}';
  }
          ...
          : Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, index) {
                        ...
                        final priceIDR = double.tryParse(item.price) ?? 0.0;
                        final rateIDR = conversionRates['IDR']!;
                        final rateTarget =
                            conversionRates[widget.selectedCurrency]!;
                        final convertedUnitPrice =
                            priceIDR / rateIDR * rateTarget;
                        final subtotal = convertedUnitPrice * item.quantity;
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(
                            '${_formatPrice(convertedUnitPrice)} x ${item.quantity} = ${_formatPrice(subtotal)}',
                          ),
                        );
                      },
                    ),
                  ...
                  Text(
                    'Total: ${_formatPrice(calculateTotalConverted(cartItems))}',
                  ...
                  DropdownButton<String>(
                    ...
                    onChanged: (value) => setState(
                      () => selectedPayment = value ?? selectedPayment,
                    ),
                  ),
                  const SizedBox(height: 16),
                  const Text(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        final total = calculateTotalConverted(cartItems);
                        ...
                        final symbol = widget.selectedCurrency == 'IDR'
                            ...
                            : widget.selectedCurrency == 'USD'
                                ...
                                : '€';
                        final message =
                            'Checkout berhasil senilai ${_formatPrice(total)}';
