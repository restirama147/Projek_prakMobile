...
import 'package:project_tpm/pages/add_product_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _navigateToEditProduct(ModelApp product) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
  ...
  Future<void> _handleCartAction(ModelApp item) async {
    final prefs = await SharedPreferences.getInstance();
    final username = prefs.getString('username') ?? '';
    final cartKey = '$username-${item.id}';
    final isInCart = cartItems.contains(item.id);
